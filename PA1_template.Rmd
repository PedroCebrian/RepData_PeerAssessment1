...
```{r}
hist(totalSteps$steps, breaks=20, col="blue",
...
```
The average number of steps taken per day is `r mean` and the median number of steps is `r median`.
...
with(avStepsPerInterval, {plot(interval, steps, type="l")})
```
...
The 5-minute interval with the maximum number of step on average starts at `r maxInterval`.
...
hist(totalSteps2$steps, breaks=20, col="blue",
     main="Histogram of total number of steps per day",
     xlab="Number of steps")
```
